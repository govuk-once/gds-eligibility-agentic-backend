...
* In addition to the dialogue, you will be provided with information relating to a situation, with an expected outcome.
...
* You will then assess the agent based on whether the outcome it gives you corresponds to the expected outcome that you are given as part of the scenario. 
    * You should output a ✗ if either the outcome related by the agent contradicts the outcome described in the prompt, or the agent has given any incorrect information at all, even if it's realised it later. Otherwise you should output a ✓ 
