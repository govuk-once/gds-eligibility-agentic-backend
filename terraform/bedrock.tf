# There is currently an issue with tf provider not associated the redshift query engine with the knowledge base
# when provisioned by terraform - knowledge base must be created manually and then associate with agent - see README
...
# Below commented out because of issue with knowledge base query engine - must associate agent and knowledge base manually
