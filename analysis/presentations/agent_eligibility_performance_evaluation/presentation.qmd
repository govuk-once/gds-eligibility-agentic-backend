...
- Eligibility rules can be complex, vague and/or incomplete
    - Part of our remit was to investigate what representation of these rules gave the best performance for the use-case of an Agentic system
...
- We are not subject-matter experts on any particular benefit, left to figure out what good looks like in naive way
    - We relied entirely on the eligibility guidance published on gov.uk to determine the relevant eligibility rules
...
- Some eligibility rules have so many edge cases a non-trivial fraction of the population falls into one or more
...
- The result of these tool calls were compared to ground truth to yield the accuracy metric over multiple trials
    - The fact that the outcomes represented an unambiguous state obviated the need for LLM-as-a-judge in discerning correctness
...
- We expanded the previous method to move from a binary representation (`ELIGIBLE`, `INELIGIBLE`) to a ternary representation of eligibility for each child (`ELIGIBLE`, `INELIGIBLE`, `INDETERMINATE`)
    - `INDETERMINATE` represents a situation where eligibility (in this case of a child for child benefit) is not explicitly covered within the rules provided (in this case taken from gov.uk)
- Additionally, an extra set of 99 test cases which represented scenarios not explicitly included within the rules were devised where the ground truth answer for at least one child was `INDETERMINATE`
...
1. How well is an agent able to apply a set of eligibility rules when conversing with a user whose situation is covered by those rules?
    * [The results demonstrate sufficient potential for the development of an Agentic eligibility service ]{style="color:teal"}
...
2. To what extent is an agent able to report `INDETERMINATE` when presented with a situation not explicitly covered by eligibility rules?
    * [The results suggest acceptable accuracy, considering the main failure modes are the most acceptable to the business]{style="color:teal"}
...
3. What representation of eligibility rules results in the best performance (i.e. accuracy, consistency, token use, latency)?
    * [The results suggest that exposing a ruleset based on the guidance already published gives the best performance for all four metrics]{style="color:teal"}
...
4. How do we best simulate a user conversation in an authentic way that controls for systematic errors relating to its artificial nature?
    * [We have answered this to an extent, but further work might be needed for completeness]{style="color:teal"}
...
All analysis, results and raw (synthetic) data can be found at [github.com/govuk-once/gds-eligibility-agentic-backend](https://github.com/govuk-once/gds-eligibility-agentic-backend/){style="text-align: center"}
