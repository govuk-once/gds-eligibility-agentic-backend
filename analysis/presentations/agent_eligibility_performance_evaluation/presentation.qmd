...
::: {.column width="40%"}
- Created a proof-of-concept representation of the child benefit eligibility rules as a traversable graph represented as JSON, where each node representing either a:
    - a question
...
- Exposed the graph to the eligibility agent via MCP tools, through which it could, for each child:
  <!-- TODO double check on what basis it traversed the graph -->
    ...
    - record a eligibility outcomes: `ELIGIBLE`, `INELIGIBLE` or `INDETERMINATE`
