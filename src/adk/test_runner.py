...
def get_or_create_output_directory(
    ...
    if resume_val == "LATEST":
        if not base_path.exists():
            raise FileNotFoundError(
                "Cannot resume. Base output directory does not exist yet."
            )

        directories = [
            d for d in base_path.iterdir() if d.is_dir() and d.name != "eval_reports"
        ]
        if not directories:
    ...
    if not output_dir.exists():
        raise FileNotFoundError(
            f"Cannot resume. Directory does not exist: {output_dir}"
        )
            ...
            if "performance" in data and "duration_seconds" in data["performance"]:
                ...
                has_final_judgement = any(
                    ...
                    in [
                        ...
                        "child_benefit_eligibility_agent_payload",
                    ]
        ...
        async with semaphore:
            ...
            for attempt in range(config["max_retries"]):
                try:
                    ...
                    # I'm building this up after seeing different errors...
                    retryable_keywords = [
                        "throttl",
                        "rate limit",
                        "429",
...
def update_token_usage(event, usage_dict: dict) -> None:
    ...
    if isinstance(usage, dict):
        ...
        p_tokens = getattr(
            usage, "prompt_token_count", getattr(usage, "prompt_tokens", 0)
        )
        c_tokens = getattr(
            usage, "candidates_token_count", getattr(usage, "completion_tokens", 0)
        )
