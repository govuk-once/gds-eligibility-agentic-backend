...
from evaluation_judge.agent import get_review_pipeline, get_original_conversation_pipeline
...
async def execute_test_case(
    ...
    app = App(
        ...
        root_agent=get_original_conversation_pipeline(test_case_without_outcome)
    )
            ...
            try:
                async for event in agen:
                    if any([part.function_response for part in event.content.parts]):
                        payload.update({f"{event.author}_payload": [part.function_response.dict() for part in event.content.parts]})
