...
def main():
    for output_dir in Path(".testOutputs").glob("*"):
        ...
        failure_df[test_case].set_index(
            ["commit", "exec_time", "permutation"], inplace=True
        )
        failure_df[test_case]["ModelSize"] = (
            failure_df[test_case]
            .index.get_level_values(0)
            .map(model_size_commit_mapping[test_case])
        )
        ...
        success_df[test_case].set_index(
            ["commit", "exec_time", "permutation"], inplace=True
        )
        success_df[test_case]["ModelSize"] = (
        ...
        combined_df_raw[test_case] = pd.concat(
            [success_df[test_case], failure_df[test_case]],
        )

        combined_df[test_case] = combined_df_raw[test_case].set_index(
            ["ModelSize"], append=True
        )
        print(combined_df[test_case].value_counts(["Passed", "ModelSize"]))
        print(
            combined_df[test_case].value_counts(
                ["ModelSize", "permutation"], ascending=True
            )
        )

        success_rates_by_permutation[test_case] = (
            100
            * combined_df[test_case][combined_df[test_case]["Passed"] == True]
            .index.get_level_values(2)
            .value_counts()
            / combined_df[test_case].index.get_level_values(2).value_counts()
        )
        print(success_rates_by_permutation[test_case].nsmallest(n=15))

        success_rates_by_permutation_model_size[test_case] = (
            100
            * combined_df[test_case][combined_df[test_case]["Passed"] == True]
            .index.droplevel(0)
            .droplevel(0)
